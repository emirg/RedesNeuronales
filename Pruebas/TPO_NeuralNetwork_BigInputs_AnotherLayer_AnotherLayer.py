...
class NeuralNetwork():
    ...
    def __init__(self):
        ...
        # Weight of second Synapse (Synapse_1) from Hidden Layer to Output Layer
        self.W4 = 2 * np.random.random((self.hiddenLayerSize, self.hiddenLayerSize)) - 1

        # Weight of second Synapse (Synapse_1) from Hidden Layer to Output Layer
        self.W5 = 2 * np.random.random((self.hiddenLayerSize, self.outputLayerSize)) - 1
        ...
        self.W5_update = np.zeros_like(self.W5)
    ...
    def train(self):
        for j in range(self.training_size):  # 100000 anduvo bastante bien para la suma
            ...
            layer_5 = self.sigmoid(np.dot(layer_4, self.W5))

            # Calculate the error
            output_error = y - layer_5  # y_j - a_j
            ...
            d = np.round(layer_5).reshape(self.binary_dim).astype('int16')
            ...
            output_layer_delta = (output_error) * self.sigmoidPrime(layer_5)

            layer_4_delta = output_layer_delta.dot(self.W5.T) * self.sigmoidPrime(layer_4)  #

            layer_3_delta = layer_4_delta.dot(self.W4.T) * self.sigmoidPrime(layer_3)  #
            ...
            self.W4_update += layer_3.T.dot(layer_4_delta)
            self.W5_update += layer_4.T.dot(output_layer_delta)
            ...
            self.W5 += self.W5_update * self.learning_rate
